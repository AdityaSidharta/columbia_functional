{-
 ...
 Due at 11:59 PM Sunday, October 10, 2021
...
nondecreasing x = all (==True) (zipWith (<=) x (tail x))
...
runLengths [] = []
runLengths (x:xs) = let (a,b) = span ( == x ) (x:xs) in (x, length a) : runLengths b
...
{- 3) Write a function "incCount" that takes an element and an
      ...
      You may assume the elements in the list are uniquenondecrease.  Yes, this
...
incCount y [] = [(y, 1)]
incCount y ((x_val, x_count):xs) | y == x_val = (x_val, x_count + 1) : xs
                                 | otherwise =  (x_val, x_count) : incCount y xs
...
[] `sorts` [] = True
x `sorts` y = nondecreasing x && (runLengths x == foldr incCount [(a, 0) | a <- uniquenondecrease x] y)
      where uniquenondecrease z = [b | (a, b) <- zip (True : zipWith (/=) z (tail z)) z, a]